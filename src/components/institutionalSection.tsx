...
export function Institutional() {
  return (
    <section id="estrutura" className="scroll-mt-[88px] md:scroll-mt-[92px] px-4 py-6 max-w-[1540px] flex flex-col mx-auto gap-8 items-start">
      <div className="items-start flex flex-col gap-4 max-w-[700px]">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold font-jakarta">Estrutura industrial para marcas que precisam envasar com eficiência e confiança</h1>
        <p className="font-manrope text-base md:text-lg font-semilight">Operamos como o braço logístico e industrial do seu negócio. Segurança operacional, agilidade e consistência são as bases da nossa entrega.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 justify-center w-full">
        {cards.map((card, index) => (
          <div key={index} className="flex flex-col items-start gap-4 max-w-[450px] bg-[var(--soft-white)] p-6 rounded-xl">
            ...
            <h3 className="text-2xl md:text-3xl font-bold font-jakarta">{card.Title}</h3>
            <p className="font-light font-manrope text-base md:text-lg ">{card.Text}</p>
          </div>
        ))}
      </div>
    </section>
